...
/**
 * JUnit 5 tests for [ManPageGenerator], [GroffFormatter], and [ManSection].
 *
 * All tests are pure (no I/O except [RealFileTests]) and deterministic.
 * No mocking needed — [ManPageGenerator.generate] is a pure function.
 */
class ManPageGeneratorTest {
    ...
    @Nested
    inner class GroffFormatterTests {
    ...
    // ── ManPageGenerator: all required sections present ──────────────────────

    @Nested
    inner class AllSectionsTests {
    ...
    // ── ManPageGenerator: arguments ──────────────────────────────────────────

    @Nested
    inner class ArgumentsTests {
    ...
    // ── ManPageGenerator: SEE ALSO ────────────────────────────────────────────

    @Nested
    inner class SeeAlsoTests {
    ...
    inner class EdgeCaseTests {
        ...
        fun `command with no arguments and no options produces minimal valid page`() {
            ...
            // Act
            val out = generate(cmd)
            // Assert — all mandatory sections present
            assertContains(out, ".SH NAME")
    ...
    // ── ManPageGenerator: file output ────────────────────────────────────────

    @Nested
    inner class RealFileTests {
